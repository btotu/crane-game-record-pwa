...
export function PrizeManager({ store, onBack, onSelectPrize }: Props) {
  ...
  const reset = () => { setName(''); setCategory('食品'); setPrice(''); setQuantity('1'); setPriceSource('user'); setUserEditedPrice(true); setEditing(null); setImageDataUrl(null); setJanCode(''); setReferenceName(''); setReferenceUrl(''); setPriceCheckedAt(todayKey()); setProductCandidates([]) }
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setError('')
    try {
      const unitPrice = Number(price || 0)
      const itemQuantity = Number(quantity)
      const input = { name, category, unitPrice, quantity: itemQuantity, estimatedPrice: unitPrice * itemQuantity, priceSource, userEditedPrice, imageDataUrl, janCode, priceReferenceName: referenceName, priceReferenceUrl: referenceUrl, priceCheckedAt }
  ...
  const applyProduct = (candidate: ProductCandidate) => {
    setName(candidate.name); setPrice(String(candidate.price)); setPriceSource('yahoo'); setUserEditedPrice(false); setReferenceName(`Yahoo!ショッピング / ${candidate.sellerName}`); setReferenceUrl(candidate.url); setPriceCheckedAt(todayKey()); setProductCandidates([]); setError(''); setFeedback('商品候補を反映しました。内容を確認してから保存してください。')
  }
