...
export function PrizeManager({ store, onBack, onSelectPrize }: Props) {
  ...
  const reset = () => { setName(''); setCategory('食品'); setPrice(''); setEditing(null); setImageDataUrl(null) }
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setError('')
    try {
      const input = { name, category, estimatedPrice: Number(price || 0), imageDataUrl }
  ...
  const startEdit = (prize: Prize) => { setEditing(prize); setName(prize.name); setCategory(prize.category); setPrice(String(prize.estimatedPrice)); setImageDataUrl(prize.imageDataUrl ?? null); setFeedback('') }
