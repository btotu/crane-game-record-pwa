...
export function PrizeManager({ store, onBack, onSelectPrize }: Props) {
  ...
  const reset = () => { setName(''); setCategory('食品'); setPrice(''); setQuantity('1'); setPriceSource('user'); setUserEditedPrice(true); setPriceBasis('市販商品価格'); setEditing(null); setImageDataUrl(null); setJanCode(''); setReferenceName(''); setReferenceUrl(''); setPriceCheckedAt(todayKey()); setProductCandidates([]) }
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setError('')
    try {
      const unitPrice = Number(price || 0)
      const itemQuantity = Number(quantity)
      const input = { name, category, unitPrice, quantity: itemQuantity, estimatedPrice: unitPrice * itemQuantity, priceSource, userEditedPrice, priceBasis, imageDataUrl, janCode, priceReferenceName: referenceName, priceReferenceUrl: referenceUrl, priceCheckedAt }
