...
export function HomeHistory({onStart}:Props){const [items,setItems]=useState<VisitSummary[]>([]);const [index,setIndex]=useState(0);const [loading,setLoading]=useState(true);const startX=useRef<number|null>(null)
 ...
 return <section className="history-carousel"><article className="visit-card" onTouchStart={e=>{startX.current=e.touches[0].clientX}} onTouchEnd={e=>{if(startX.current===null)return;const dx=e.changedTouches[0].clientX-startX.current;if(Math.abs(dx)>45)move(index+(dx>0?1:-1));startX.current=null}}>
