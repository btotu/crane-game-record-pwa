...
export const playService = {
  ...
  update(id:string,input:PlayUpdateInput){if(!Number.isInteger(input.amount)||input.amount<=0||input.amount>1000000)throw new Error('使用額は1～1,000,000円の整数で入力してください。');if(input.memo.trim().length>200)throw new Error('メモは200文字以内で入力してください。');return playRepository.update(id,input)},
