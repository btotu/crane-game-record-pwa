...
export const playService = {
  ...
  async update(id:string,input:PlayUpdateInput){const dateTime=new Date(`${input.playDate}T${input.playTime}:00`);if(!input.playDate||!input.playTime||Number.isNaN(dateTime.getTime()))throw new Error('日付と時刻を正しく入力してください。');if(dateTime.getTime()>Date.now())throw new Error('未来の日時は登録できません。');if(!Number.isInteger(input.amount)||input.amount<=0||input.amount>1000000)throw new Error('使用額は1～1,000,000円の整数で入力してください。');if(!Number.isInteger(input.estimatedPriceAtPlay)||input.estimatedPriceAtPlay<0||input.estimatedPriceAtPlay>1000000)throw new Error('保存時の参考価格は0～1,000,000円の整数で入力してください。');if(input.memo.trim().length>200)throw new Error('メモは200文字以内で入力してください。');const previous=await playRepository.get(id);if(!previous)throw new Error('編集対象のプレイ記録が見つかりません。');await playRepository.update(id,input);return previous},
  remove: playRepository.remove,
  restore: playRepository.restore,
}
