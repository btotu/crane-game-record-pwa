...
}, async details(storeId:string,playDate:string):Promise<VisitPlayDetail[]>{const [plays,prizes]=await Promise.all([playRepository.forVisit(storeId,playDate),prizeService.list()]);const map=new Map(prizes.map(item=>[item.id,item]));return plays.map(play=>{const prize=map.get(play.prizeId);return{id:play.id,prizeId:play.prizeId,prizeName:prize?.name??'削除済み景品',prizeImageDataUrl:prize?.imageDataUrl,category:prize?.category??'その他',amount:play.amount,result:play.result,startedAt:play.startedAt,estimatedValue:play.result==='撤退'?0:(play.estimatedPriceAtPlay??prize?.estimatedPrice??0),memo:play.memo??'',isApproximate:play.isApproximate}})} }
