...
export const visitService = { async list():Promise<VisitSummary[]> {
  ...
  for(const play of plays){ const key=`${play.playDate}:${play.storeId}`; const won=play.result!=='撤退'; const value=won?(prizeMap.get(play.prizeId)?.estimatedPrice??0):0; const current=grouped.get(key)
    ...
    else grouped.set(key,{id:key,storeId:play.storeId,storeName:storeMap.get(play.storeId)?.name??'削除済み店舗',playDate:play.playDate,startedAt:play.startedAt,totalSpent:play.amount,estimatedPrizeValue:value,profit:value-play.amount,acquiredCount:won?1:0,playCount:1,isApproximate:play.isApproximate}) }
